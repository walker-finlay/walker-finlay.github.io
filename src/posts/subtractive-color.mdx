...
$$
\begin{align}
p_i(x)&= \frac{1}{4}+\frac{1}{4}\sin\left(\frac{2\pi}{w_f}\left\lfloor\frac{x}{w_s}\right\rfloor w_s\right) 
\\
\mathbf{s} &=
\begin{bmatrix}
s_r & s_g & s_b
...
\begin{align}
  \text{clip} &= \mathbf{c} =
    ...
    \end{bmatrix} =
    \begin{bmatrix}
      1 \text{ if } (s_i-w_f/2 \leq c_r \lt s_i+w_f/2) \text{ else } 0 \\
      \vdots
    \end{bmatrix}
  \\ 
  \text{filters} &= A =
...
ctx.fillStyle = `rgb(${filter_base
  .map(
    (row, idx_rgb) => row.map(  // (4), (5)
      el => clip[idx_rgb] && el && p(delims[i] - fw / 2 - sliders[idx_rgb], bw, fw)
    )
  )
