...
The demo above simulates a Felipe Pantone piece in which pieces of glass painted red, green, and blue are placed in front of a backlight. Each is segmented into a range of intensities. The segments are defined by a floored $\sin$ function $p(x)$, a matrix `filter_base`, which defines the light blocked by each filter, and an RGB to CMYK conversion function $f(x)$. For example, at $x=0$, the red filter allows $p(0)$ magenta and yellow light to pass through.
...
<iframe src="https://www.desmos.com/calculator/b9eajsoomb" width="100%" style="min-height:400px"></iframe>
...
1. Composing the filter base with the color intensity function
