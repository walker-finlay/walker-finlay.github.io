...
Printers use subtractive color because their medium is physical paper with (usually) white light reflecting off of it. Colors are simulated by combining cyan, magenta, yellow, and black inks. This scheme will be the basis for the simulation below.
...
- it is not any more expensive to pre-calculate these, it will have to be 
done no matter what. However, if the segments got small enough, memory would 
become a consideration, and they would have to be computed one by one
*/
